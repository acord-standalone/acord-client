...
export const DATA_DIR = process.env.ACORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "AcordData", suffix)
        : join(app.getPath("userData"), "..", "Acord", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[Acord] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
